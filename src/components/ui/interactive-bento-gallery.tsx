...
const GalleryModal = ({ selectedItem, isOpen, onClose, setSelectedItem, mediaItems }: GalleryModalProps) => {
  ...
  return (
    <>
      ...
      >
        <motion.div className="relative rounded-xl bg-[var(--color-accent-soft)]/80 backdrop-blur-xl border border-[var(--color-border)] shadow-lg cursor-grab active:cursor-grabbing">
          <div className="flex items-center -space-x-2 px-3 py-2">
            {mediaItems.map((item, index) => (
              <motion.div
                ...
                className={`relative group w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 flex-shrink-0 rounded-lg overflow-hidden cursor-pointer hover:z-20 ${selectedItem.id === item.id ? "ring-2 ring-[var(--color-primary)] shadow-lg" : "hover:ring-2 hover:ring-[var(--color-primary)]/30"}`}
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      <div className="mb-8 text-center">
        ...
        <motion.p
          className="mt-2 text-sm sm:text-base text-[var(--color-text-muted)]"
      ...
      <AnimatePresence mode="wait">
        {selectedItem ? (
          <GalleryModal selectedItem={selectedItem} isOpen={true} onClose={() => setSelectedItem(null)}
          ...
          >
            {items.map((item, index) => (
              <motion.div
                key={item.id} layoutId={`media-${item.id}`}
                ...
                onDragEnd={(_, info) => {
                  ...
                  if (Math.abs(d) > 50) {
                    ...
                    next.splice(index, 1); next.splice(target, 0, dragged);
                    setItems(next);
                  }
                }}
              >
